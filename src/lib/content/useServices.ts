...
import { Lightbulb, Layers, Globe, Code2, Search, Palette, type LucideIcon } from "lucide-react";
...
function internalAnchorText(href: string | null, text: string | null, fallback: string) {
  ...
  if (target === "/services/graphic-design" && current === "view design portfolio") {
    return "Graphic Design and Branding Dubai";
  }
    ...
    queryFn: async () => {
      ...
      return (data as unknown as Row[]).map((row, i) => {
        ...
        return {
          ...
          to: row.href ?? fb.to,
          cta: internalAnchorText(row.href, row.cta_text, fb.cta),
        };
      });
    },
  });
